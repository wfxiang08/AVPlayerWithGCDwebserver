...
extern "C" {
    ...
    #define NIDASSERT(xx) { \
        if (!(xx)) { \
            NIDPRINT(@"NIDASSERT failed: %s", #xx); \
    ...
    // 只在Debug模式下生效
    #define NIDASSERT(xx) { \
        if (!(xx)) { NIDPRINT(@"NIDASSERT failed: %s", #xx); \
            if (NIDebugAssertionsShouldBreak && NIIsInDebugger()) { \
                raise(SIGTRAP); \
            } \
        } \
 ...
 */
#if defined(DEBUG) || defined(NI_DEBUG)
    #define NIDPRINT(xx, ...)  NSLog(@"%s(%d): " xx, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
